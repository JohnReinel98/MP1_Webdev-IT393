...
//Page number
$this->Cell(0,10,'Page '.$this->PageNo().' Package Delivery Receipt',0,0,'C');
}
function LoadData()
{
$packID=$_SESSION['recptsession'];
$result=mysql_query("select * from tblpackage_delivery where TrackNo='$packID'");
...
$declaredval = $rows['DeclaredValue'];
$fee = $rows['Amount'];
$size = $rows['Size'];
$branch = $rows['Branch'];
}
...
$this->Ln();
$this->Cell($w[0],6,"Declared Value: ".$declaredval);
$this->Ln();
$this->Cell($w[0],6,"Size: ".$size);
$this->Ln();
